...
@section('content_header')
    <h1 class="m-0 text-dark">Ubah Aktivitas</h1>
@stop

@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ route('detailRab.update', ['id' => $detail->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('patch')
                <div class="row row-cols-3 ">
                    <div class="col mb-3">
                        ...
                        <input type="text" class="form-control" name="aktivitas" id="aktivitas"
                            placeholder="Nama Aktivitas" value="{{ $detail->aktivitas }}" required>
                    </div>
                    <div class="col mb-3">
                    ...
                    <div class="col mb-3">
                        ...
                        <input type="text" class="form-control" name="penyedia" id="penyedia" placeholder="Penyedia"
                            value="{{ $detail->penyedia }}" required>
                    </div>
                    <div class="col mb-3">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control" name="iku" id="iku" required>
                        @foreach ($iku as $i)
                            <option value="{{ $detail->iku_id }}|{{ $detail->iku_text }}" selected> {{ $detail->iku_text }}
                            </option>
...
@section('js')
    ...
    <script>
        let namaPeserta = JSON.parse(@json($detail->nama_peserta));
        ...
        let selectedAkun = @json($detail->akunDetail);
        let akun = {}
        selectedAkun.forEach(data => {
            let id = "sbm_" + data.sbm_id;
            if (!akun.hasOwnProperty(id)) {
                akun[id] = {};
                akun[id].sbm_id = data.sbm_id;
                akun[id].sbm_text = data.sbm_text;
                akun[id].data = [];
            }
            d = {}
            d.modal_sbm_id = data.sbm_id
            d.modal_sbm_text = data.sbm_text
            d.modal_sbm_detail_id = data.sbm_detail_id
            d.modal_sbm_detail_text = data.sbm_detail_text
            d.modal_jumlah = data.jumlah
            d.modal_jumlah_satuan = data.jumlah_satuan
            d.modal_jam = data.jam
            d.modal_jam_satuan = data.jam_satuan
            d.modal_frek = data.frek
            d.modal_frek_satuan = data.frek_satuan
            d.modal_total = data.total
            d.modal_total_satuan = data.total_satuan
            d.modal_harga_satuan = data.harga_satuan
            akun[id].data.push(d);
        });
